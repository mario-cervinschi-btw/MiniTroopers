...
import { combineLatest, debounceTime, distinctUntilChanged, finalize, switchMap, tap } from 'rxjs';
...
export class CompaniesTableSignalsComponent {
  ...
  protected readonly showX = computed(() => {
  ...
  private readonly companiesResource$ = combineLatest({
    page: toObservable(this.pageIndex).pipe(distinctUntilChanged()),
    limit: toObservable(this.itemsPerPage).pipe(distinctUntilChanged()),
  ...
  protected readonly isEmpty = computed(() => {
    return this.companiesAvailable().length === 0;
  });
