...
let UsersService = class UsersService {
    ...
    async findAll(search, sort, order, page, limit) {
        const query = this.usersRepository.createQueryBuilder('user');
        query.leftJoinAndSelect('user.skills', 'skill');
        if (search) {
            query.where('LOWER(user.firstName) LIKE :search OR LOWER(user.lastName) LIKE :search OR LOWER(skill.name) LIKE :search', { search: `%${search.toLowerCase()}%` });
        }
